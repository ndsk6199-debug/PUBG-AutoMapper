...
if ($obj.coordinateSystem.type -ne 'normalized') { throw 'coordinateSystem.type must be normalized.' }
function Test-Point($name, $p) {
  if ($null -eq $p) { throw "Missing point: $name" }
  if ($p.x -lt 0 -or $p.x -gt 1 -or $p.y -lt 0 -or $p.y -gt 1) {
    throw "Out-of-range normalized coordinate for ${name}: $($p.x),$($p.y)"
  }
}
...
if ($obj.input.movement.keys.W -ne 'up' -or $obj.input.movement.keys.A -ne 'left' -or $obj.input.movement.keys.S -ne 'down' -or $obj.input.movement.keys.D -ne 'right') {
  throw 'WASD movement mapping is invalid.'
}
